...
BEGIN	
	...
	tmp_situacionRevista AS (
		SELECT le.id AS legajoid ,max(suno.codigo) AS situacionrevistauno,max(sdos.codigo) AS situacionrevistados,max(stres.codigo) AS situacionrevistatres
		FROM Legajo le
	...
	LEFT(TRANSLATE(REPLACE(coalesce((upper(l.apellido || ' ' || l.nombre)),''), 'Ñ', 'N'), 'áéíóúÁÉÍÓÚäëïöüÄËÏÖÜ','aeiouAEIOUaeiouAEIOU') || C_ESPACIOS, 30) AS NombreApellido,
	...
	RIGHT(C_ZEROS || coalesce(max(tsr.situacionrevistauno),''), 2) AS SituacionRevista1,
	RIGHT(C_ZEROS || coalesce(to_char(max(li.fechasituacionrevistauno),'DD'),''), 2) AS DiaInicioSituacionRevista1,
	RIGHT(C_ZEROS || coalesce(max(tsr.situacionrevistados),''), 2) AS SituacionRevista2,
	RIGHT(C_ZEROS || coalesce(to_char(max(li.fechasituacionrevistados),'DD'),''), 2) AS DiaInicioSituacionRevista2,
	RIGHT(C_ZEROS || coalesce(max(tsr.situacionrevistatres),''), 2) AS SituacionRevista3,
	RIGHT(C_ZEROS || coalesce(to_char(max(li.fechasituacionrevistatres),'DD'),''), 2) AS DiaInicioSituacionRevista3,
	...
	LEFT JOIN tmp_situacionRevista tsr ON l.id = tsr.legajoid
